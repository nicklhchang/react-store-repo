...
const Menu = function () {
    ...
    const logOutUser = useCallback(function () {
        ...
        setWholeMenu([]);
        setMenuPage([]);
    }, [clearFilterOptions, setWholeMenu, unauthenticate, setMenuPage]) // setMenuPage from this component
    ...
    useEffect(() => {
        ...
        // empty dep array makes it run only on initial render and never again; wanted
        // because if not empty, any state change (this functional component or in context)
        // will trigger this useEffect() to run, which messes with display
        // eslint-disable-next-line react-hooks/exhaustive-deps
        []);

    useEffect(() => {
        console.log('running custom menu useeffect')
        const controller = new AbortController();
        if (Object.keys(sidebarFilterOptions).length) {
            ...
            if (mealTypes && budgetPrice) { // get around first render
        ...
        // , handleAxiosGetThen, axiosReqLink, clearFilterOptions
        // eslint-disable-next-line react-hooks/exhaustive-deps
        [sidebarFilterOptions]);
